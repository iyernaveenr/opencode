...
let getSessionTerminalCacheKey: typeof import("./context").getSessionTerminalCacheKey
let getTerminalCacheKey: typeof import("./context").getTerminalCacheKey
...
describe("getWorkspaceTerminalCacheKey", () => {
  ...
  test("session terminals get their own key per chat session", () => {
    expect(String(getSessionTerminalCacheKey("/repo", "ses_a"))).toBe("local\u0000/repo\u0000session:ses_a")
    expect(String(getSessionTerminalCacheKey("/repo", "ses_a"))).not.toBe(
      String(getSessionTerminalCacheKey("/repo", "ses_b")),
    )
  })

  test("drafts without a session fall back to the workspace key", () => {
    expect(String(getTerminalCacheKey("/repo", undefined))).toBe(String(getWorkspaceTerminalCacheKey("/repo")))
    expect(String(getTerminalCacheKey("/repo", "ses_a"))).toBe(String(getSessionTerminalCacheKey("/repo", "ses_a")))
  })
